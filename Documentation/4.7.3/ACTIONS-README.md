...
4. [Test operators](#4-test-operators)
	1. [name(pattern)](#41-namepattern)
	2. [pathname(pattern)](#42-pathnamepattern)
	3. [subpathname(pattern)](#43-subpathnamepattern)
	4. [filesize(value)](#44-filesizevalue)
	5. [tailsize(value)](#45-tailsizevalue)
	6. [dirsize(value)](#46-dirsizevalue)
	7. [size(value)](#47-sizevalue)
	8. [inode(value)](#48-inodevalue)
	9. [nlink(value)](#49-nlinkvalue)
	10. [fileblocks(value)](#410-fileblocksvalue)
	11. [dirblocks(value)](#411-dirblocksvalue)
	12. [blocks(value)](#412-blocksvalue)
	13. [uid(value)](#413-uidvalue)
	14. [gid(value)](#414-gidvalue)
	15. [user(string)](#415-userstring)
	16. [group(string)](#416-groupstring)
	17. [depth(value)](#417-depthvalue)
	18. [dircount(value)](#418-dircountvalue)
	19. [filesize_range(minimum, maximum)](#419-filesize_rangeminimum-maximum)
	20. [tailsize_range(minimum, maximum)](#420-tailsize_rangeminimum-maximum)
	21. [dirsize_range(minimum, maximum)](#421-dirsize_rangeminimum-maximum)
	22. [size_range(minimum, maximum)](#422-size_rangeminimum-maximum)
	23. [inode_range(minimum, maximum)](#423-inode_rangeminimum-maximum)
	24. [fileblocks_range(minimum, maximum)](#424-fileblocks_rangeminimum-maximum)
	25. [dirblocks_range(minimum, maximum)](#425-dirblocks_rangeminimum-maximum)
	26. [blocks_range(minimum, maximum)](#426-blocks_rangeminimum-maximum)
	27. [uid_range(minimum, maximum)](#427-uid_rangeminimum-maximum)
	28. [gid_range(minimum, maximum)](#428-gid_rangeminimum-maximum)
	29. [depth_range(minimum, maximum)](#429-depth_rangeminimum-maximum)
	30. [dircount_range(minimum, maximum)](#430-dircount_rangeminimum-maximum)
	31. [type(c)](#431-typec)
	32. [perm(mode)](#432-permmode)
	33. [file(string)](#433-filestring)
	34. [exists()](#434-exists)
	35. [absolute()](#435-absolute)
	36. [readlink(expression)](#436-readlinkexpression)
	37. [eval(path, expression)](#437-evalpath-expression)
	38. [false](#438-false)
	39. [true](#439-true)
5. [Actions](#5-actions)
	...
	2. [Actions applied at directory scanning (stage 2)](#52-actions-applied-at-directory-scanning-stage-2)
		...
		3. [no-fragments()](#523-no-fragments)
		4. [tailend()](#524-tailend)
		5. [no-tailend()](#525-no-tailend)
		6. [compressed()](#526-compressed)
		7. [uncompressed()](#527-uncompressed)
		8. [uid(uid or user)](#528-uiduid-or-user)
		9. [gid(gid or group)](#529-gidgid-or-group)
		10. [guid(uid/user, gid/group)](#5210-guiduiduser-gidgroup)
		11. [chmod(mode)](#5211-chmodmode)
		12. [align(value)](#5212-alignvalue)
	3. [Actions applied at second directory scan (stage 3)](#53-actions-applied-at-second-directory-scan-stage-3)
		1. [prune()](#531-prune)
	4. [Actions applied at third directory scan (stage 4)](#54-actions-applied-at-third-directory-scan-stage-4)
		1. [empty(reason)](#541-emptyreason)
	5. [Actions performed at filesystem creation (stage 6)](#55-actions-performed-at-filesystem-creation-stage-6)
		1. [xattrs-exclude(regex)](#551-xattrs-excluderegex)
		2. [xattrs-include(regex)](#552-xattrs-includeregex)
		3. [xattrs-add(name=val)](#553-xattrs-addname=val)
...
- ```<``` match on less than number
- ```<=``` match on less than or equal to number
- "" (nothing), match on exactly number
- ```>=``` match on greater than or equal to number
- ```>``` match on greater than number

```-``` can be used in place of ```<``` and ```+``` can be used in place of ```>``` for
compatibility with the find command.
...
Return TRUE if the size of the tail end of the file is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Returns FALSE on anything not a file.

### 4.6 dirsize(value)

Return TRUE if the size of the directory is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Returns FALSE on anything not a directory.

### 4.7 size(value)

Return TRUE if the size of the file is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Works on any file type.

### 4.8 inode(value)

Return TRUE if the inode number is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Works on any file type.

### 4.9 nlink(value)

Return TRUE if the nlink count is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Works on any file type.

### 4.10 fileblocks(value)

Return TRUE if the size of the file in blocks (512) is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Works on any file type.

### 4.11 dirblocks(value)

Return TRUE if the size of the directory in blocks (512) is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Returns FALSE on anything not a directory.
...
Return TRUE if the uid value is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Works on any file type.

### 4.14 gid(value)

Return TRUE if the gid value is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Works on any file type.
...
Return TRUE if the file is at depth less than, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Works on any file type.

### 4.18 dircount(value)

Return TRUE if the number of files in the directory is smaller, larger or equal
to ```<value>```, where ```<value>``` can be ```<number```, ```<=number```, ```number```, ```>=number```, ```>number```.
Returns FALSE on anything not a directory.

### 4.19 filesize_range(minimum, maximum)

Return TRUE if the size of the file is within the range ```[<minimum>, <maximum>]```
inclusive.  Returns FALSE on anything not a file.

### 4.20 tailsize_range(minimum, maximum)
...
### 4.21 dirsize_range(minimum, maximum)
...
- perm(mode) - TRUE if file's permission bits are exactly ```<mode>```.  ```<mode>``` can be octal or symbolic.

- perm(-mode) - TRUE if all ```<mode```> permission bits are set for this file.  ```<mode>``` can be octal or symbolic.

- perm(/mode) - TRUE if any ```<mode>``` permission bits are set for this file. ```<mode>``` can be octal or symbolic.

The symbolic mode is of the format ```[ugoa]*[[+-=]PERMS]+```. PERMS = ```[rwxXst]+``` or ```[ugo]``` and can be repeated separated with commas.
...
Exclude any files/directories belonging to user phillip
...
Exclude any regular files larger than 1M

```
exclude@depth(>3)
```

Only archive files/directories to a depth of 3

```
exclude@depth(>3) || (depth(3) && type(d))
```

As above but also exclude directories at the depth of 3
...
The symbolic mode is of the format ```[ugoa]*[[+-=]PERMS]+```.  PERMS = ```[rwxXst]+``` or ```[ugo]```
...
Align all files to a 4K boundary.

```
align(32K)@filesize(>=32K)
```
...
First, in the first stage Mksquashfs is building an in-memory representation
...
Second, many exclusion/pruning operations may only be easily applied after
...
Exclude any file which isn't a directory, and then clean-up
