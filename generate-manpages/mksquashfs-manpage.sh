...
if [ ! -x $1/mksquashfs ]; then
	echo "$0: <arg1> doesn't point to a directory with Mksquashfs in it!" 2>&1
	echo "$0: <arg1> should point to the directory with the Mksquashfs" \
...
if ! $1/mksquashfs -help > $tmp/mksquashfs.help2 2>&1; then
	echo "$0: Running Mksquashfs failed.  Cross-compiled or incompatible binary?" 2>&1
	exit 1
fi
...
echo >> $tmp/mksquashfs.version
echo "Written by Phillip Lougher <phillip@squashfs.org.uk>" >> $tmp/mksquashfs.version
